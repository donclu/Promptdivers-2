...
1. **Project root** — `${CLAUDE_PLUGIN_ROOT}/AGENTS.md`, `${CLAUDE_PLUGIN_ROOT}/squads/`, `${CLAUDE_PLUGIN_ROOT}/protocols/`, `${CLAUDE_PLUGIN_ROOT}/docs/` in the current workspace.
2. **Vendored framework** — `.framework-promptdivers2/` (or the path declared in the project `${CLAUDE_PLUGIN_ROOT}/AGENTS.md` stub) when the pack was installed with `--vendor-framework`.
...
5. If `COMMS_MODE: RADIO` is set, use radio-style SITREPs from `${CLAUDE_PLUGIN_ROOT}/protocols/radio-comms.md` — but never drop evidence.
...
STEP 0 — Planet check (pre-drop):
  → Read GALACTIC_WAR_MAP.md / PROJECT_LOG.md / ${CLAUDE_PLUGIN_ROOT}/AGENTS.md debt section
  → Identify active fronts + hottest sector + threat level → squad + nave
  → See ${CLAUDE_PLUGIN_ROOT}/protocols/pre-drop.md for full procedure

No ${CLAUDE_PLUGIN_ROOT}/AGENTS.md or no project brief?
...
## Stratagem routing (concrete actions — check `${CLAUDE_PLUGIN_ROOT}/stratagems/` when available)
...
```
THE AUTHENTIC   — Load ${CLAUDE_PLUGIN_ROOT}/AGENTS.md + log. Define scope. Brief the next role.
...
THE SCRIBE      — PROJECT_LOG + changelog + ${CLAUDE_PLUGIN_ROOT}/AGENTS.md updates.
...
| **agent-context** | Missing or stale ${CLAUDE_PLUGIN_ROOT}/AGENTS.md |
...
0. **Planet check (pre-drop).** If `GALACTIC_WAR_MAP.md` exists in the project, read it. Otherwise check `PROJECT_LOG.md` or `${CLAUDE_PLUGIN_ROOT}/AGENTS.md` debt section. Identify active fronts (Terminids / Automatons / Illuminate), hottest sector, and threat level. Derive squad + nave recommendation. Fill the "Planet status" block in `NEXT_MISSION.md` if one is being written. If no planet state exists: assume UNEXPLORED terrain → Squad A. See `${CLAUDE_PLUGIN_ROOT}/protocols/pre-drop.md` for full procedure.
...
2. Select nave per routing table or ${CLAUDE_PLUGIN_ROOT}/AGENTS.md declaration.
3. Say one line: next file or skill to load.
4. Load it, execute phase, SITREP when useful.
5. When phase ends with fixed objectives, run `${CLAUDE_PLUGIN_ROOT}/protocols/mission-debrief.md` or load **promptdivers-pelican**. If PARTIAL/FAIL, apply stratagem map before next phase.
