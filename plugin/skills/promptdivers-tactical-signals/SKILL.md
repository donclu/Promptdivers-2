...
**Path resolution:** Look for `${CLAUDE_PLUGIN_ROOT}/protocols/tactical-signals.md` in workspace root → `.framework-promptdivers2/` (or vendored path in project `${CLAUDE_PLUGIN_ROOT}/AGENTS.md`). If found, read it for the full signal table and examples. If not found, use the compact grid below.
...
🚨 ALERT — sensitive data sent to cloud model — verify ${CLAUDE_PLUGIN_ROOT}/AGENTS.md permissions
```

Note: on the **main thread**, "switching nave" is only real if a human ran `/model`, or if the switch actually happened inside a spawned subagent/`Workflow` call. A SITREP claiming a self-switch with neither behind it is a narrated switch, not a real one — see `${CLAUDE_PLUGIN_ROOT}/docs/claude-code-model-execution.md`.
