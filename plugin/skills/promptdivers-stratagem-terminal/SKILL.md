...
**Primary references (when available — workspace root, then `.framework-promptdivers2/` or vendored path in project `${CLAUDE_PLUGIN_ROOT}/AGENTS.md`):**

- `${CLAUDE_PLUGIN_ROOT}/QUICK_REFERENCE.md` (loadout table)
- `${CLAUDE_PLUGIN_ROOT}/stratagems/README.md` (full codex)
- `${CLAUDE_PLUGIN_ROOT}/protocols/mission-debrief.md` (failure routing)
...
📍 MARK — Read next:
- ${CLAUDE_PLUGIN_ROOT}/QUICK_REFERENCE.md (loadout)
- ${CLAUDE_PLUGIN_ROOT}/stratagems/<category>/<file>.md
```
