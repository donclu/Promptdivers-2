...
- **[H-1] Inconsistencia `country↔branch_code`**
  - Evidencia: `country_branch_mismatch = {country_branch_mismatch}`
...
- **[H-2] Fechas inválidas / no parseables**
  - Evidencia: `date_parse_fail = {date_parse_fail}`
...
- **[M-1] `invoice_id` faltante o duplicado**
  - Evidencia: missing = `{invoice_id_missing}`, dupes = `{invoice_id_dupe}`
...
- **[M-2] `unit_price` / `qty_or_hours` con formatos no estándar**
  - Evidencia: unit_price parse fails = `{unit_price_parse_fail}`, qty parse fails = `{qty_parse_fail}`
...
- **[L-1] Invisibles / mojibake en `customer_name`**
  - Evidencia: invisibles = `{invisibles_in_customer_name}`, mojibake = `{mojibake_in_customer_name}`
